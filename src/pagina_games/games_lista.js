      ...
      const JuegosList = ({ juegos, selectedPlatform}) => {
        ...
        const getPlataformas = async (juegoId) => {
          try {
            const { data } = await axios.get(`https://api.rawg.io/api/games/${juegoId}/platforms?key=84dafbf2540d47389fc971ea576592fa`);
            return data.results;
          } catch (error) {
                    ...
                    <div key={juego.id} className="juego-bloque">
                      ...
                      <div className="game-card">
                        <div className="game-card-info">
                                  ...
                                  return platformIcon ? (
                                    <li key={plataforma.platform.id} className="plataforma-estructura">
                                      <img
                                        ...
                                        title={plataforma.platform.name} // Agregar título para descripción emergente
                                      />
                                    </li>
                                  ) : null;
                                })
                          ...
                          {juego.genres.map((g) => g.name).join(' ')}</p>}
                          {/* Agrega una clase condicional para cambiar el color del rating */}
